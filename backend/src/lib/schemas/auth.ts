...
export const AuthConfigResponse = z.object({
  ...
  data: z.object({
    authMethods: z.array(z.string()).describe("Enabled auth methods: email_password, oauth_google, oauth_apple, magic_link, passkey"),
...
export const MagicLinkVerifyRequest = z.object({
  token: z.string(),
...
export const EmailVerifySendRequest = z.object({
...
export const MfaMethodsResponse = z.object({
  success: z.literal(true),
  data: z.object({
    available: z.array(z.enum(["totp", "email"])).describe("MFA methods available for enrollment"),
...
export const MfaConfirmRequest = z.object({
...
export const PasskeyOptionsResponse = z.object({
  success: z.literal(true),
  data: z.object({}).passthrough().describe("WebAuthn options (challenge, rpId, etc.)"),
}).openapi("PasskeyOptionsResponse");

export const PasskeyVerifyRequest = z.object({}).passthrough().describe("WebAuthn credential response from browser").openapi("PasskeyVerifyRequest");
