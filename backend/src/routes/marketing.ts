...
const listCampaignsRoute = createRoute({
  ...
  responses: {
    200: {
      content: { "application/json": { schema: CampaignsListResponse } },
      description: "Campaigns",
    },
  },
});
marketing.openapi(listCampaignsRoute, (c) =>
  c.json({ success: true as const, data: { campaigns: [] } }),
);
...
marketing.openapi(createPhoneRoute, (c) =>
  c.json({ success: true as const, data: { campaign: null } }),
);
...
marketing.openapi(createEmailRoute, (c) =>
  c.json({ success: true as const, data: { campaign: null } }),
);
...
marketing.openapi(getCampaignRoute, (c) =>
  c.json({ success: true as const, data: { campaign: null } }),
);
...
const pauseRoute = createRoute({
  ...
  request: { params: z.object({ id: z.string() }) },
  responses: {
    200: { content: { "application/json": { schema: SuccessResponse } }, description: "Paused" },
  },
});
marketing.openapi(pauseRoute, (c) =>
  c.json({ success: true as const, data: { message: "Campaign paused" } }),
);
...
marketing.openapi(analyticsRoute, (c) =>
  c.json({ success: true as const, data: { analytics: null } }),
);
