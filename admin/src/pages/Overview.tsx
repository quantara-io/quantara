...
export function Overview() {
  ...
  if (error) return <div className="p-4 rounded bg-red-950/40 text-red-300 border border-red-900 text-sm">{error}</div>;
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <Hero label="Fear & Greed" value={fg ? `${fg.value}` : "—"} sub={fg?.classification ?? ""} valueClass={fgColor} />
        <Hero label="Ingestion" value={`${data.ecsStatus.running}/${data.ecsStatus.desired}`}
          sub={data.ecsStatus.status} valueClass={ecsHealthy ? "text-emerald-400" : "text-red-400"} />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
        <Card title="DynamoDB Tables">
          <Table headers={["Table", "Count", "Size"]} rows={data.tableCounts.map((t) => [
            t.name, t.count < 0 ? "—" : t.count.toLocaleString(), `${(t.size / 1024 / 1024).toFixed(2)} MB`,
          ])} />
        </Card>

        <Card title="SQS Queues">
          <Table headers={["Queue", "Messages", "In-flight"]} rows={data.queueDepths.map((q) => [
            q.dlq ? `${q.name} ⚠` : q.name,
        ...
        <Card title="Lambda Functions">
          <Table headers={["Function", "State", "Size"]} rows={data.lambdaStatuses.map((l) => [
            l.name, l.state, l.size ? `${(l.size / 1024).toFixed(0)} KB` : "—",
          ])} />
        </Card>
      ...
      <p className="text-xs text-slate-600">Updated {new Date(data.timestamp).toLocaleTimeString()} · refreshes every 30s</p>
    </div>
  );
}
...
function Hero({ label, value, sub, valueClass }: { label: string; value: string; sub?: string; valueClass?: string }) {
  return (
...
function Table({ headers, rows }: { headers: string[]; rows: (string | number)[][] }) {
  return (
    <table className="w-full text-xs">
      <thead>
        <tr className="text-slate-500">{headers.map((h) => <th key={h} className="text-left font-medium pb-2">{h}</th>)}</tr>
      </thead>
      <tbody>
        {rows.map((r, i) => (
          <tr key={i} className="border-t border-slate-800">
            {r.map((v, j) => <td key={j} className="py-1.5 text-slate-300">{v}</td>)}
          </tr>
        ))}
      </tbody>
    </table>
